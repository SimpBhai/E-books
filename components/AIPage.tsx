...
export default function AIPage({ onBack }: { onBack: () => void }) {
  ...
  async function ask(event: React.FormEvent) {
    ...
    try {
      const response = await fetch('/api/ai/answer', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ question }) });
  ...
  return <main className="min-h-screen bg-stone-50 text-stone-900 px-4 py-8 md:px-12"><div className="mx-auto max-w-3xl">
    ...
    <form onSubmit={ask} className="mt-10 flex flex-col gap-4 rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
      <label className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-stone-500"><KeyRound size={14}/> Bot/API key</label><input value={apiKey} onChange={e => setApiKey(e.target.value)} type="password" required placeholder="Bearer key" className="rounded-lg border border-stone-200 bg-stone-50 px-4 py-3 outline-none focus:border-ochre-500" />
