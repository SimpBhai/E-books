...
import { Menu, Search, Moon, Sun, ChevronLeft, ChevronRight, Share2, Library, ArrowRight, Bookmark as BookmarkIcon, Check, Filter, BookOpen, Globe } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // Content View State
  // Single state to control language for both Sutrarth and Bhavarth
  ...
  useEffect(() => {
    if (selectedBook) {
      const bookChapters = getBookContent(selectedBook.id);
      const bookVerses = getAllVerses(selectedBook.id);
  ...
  const handleBookmarkNavigation = (bookId: string, verseId: string) => {
    const book = getBookMetadata(bookId);
    if (book) {
      setSelectedBook(book);
      const targetVerse = getVerseById(bookId, verseId);
      if (targetVerse) {
  ...
  // Derive all available languages for the current verse to show in the global toggle
  ...
  if (!selectedBook) {
    return (
      <div className={`min-h-screen w-full ${darkMode ? 'dark bg-stone-900 text-stone-100' : 'bg-stone-50 text-stone-900'}`}>
        ...
        <BookmarksPanel 
          ...
          getBookDetails={getBookMetadata}
          getVerseDetails={getVerseById}
        />
      </div>
    );
  }
  ...
  // Reader View
  return (
    <div className={`flex h-screen w-full overflow-hidden ${darkMode ? 'dark bg-stone-900 text-stone-100' : 'bg-stone-50 text-stone-900'}`}>
      <Sidebar 
